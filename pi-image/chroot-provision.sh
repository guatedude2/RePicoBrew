#!/bin/bash
# Provisioning for the image (current Raspberry Pi OS Lite, arm64, i.e. Trixie, which uses NetworkManager).
# Runs INSIDE a chroot into the image's root partition with real network access (called from build.sh on
# the build host — never on the Pi itself). It mirrors what scripts/deploy-to-pi.sh does to a stock Raspberry Pi OS,
# so an image and a deployed Pi end up the same.
#
